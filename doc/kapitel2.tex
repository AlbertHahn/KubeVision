...
Der hauptsächliche Verwendungszweck von k3s liegt in IoT und Edge-Devices, da viele Treiber ersetzt wurden \cite{k3s}.
...
weitergeleitet wird. Desweiteren verfügt auch die Master Node über Kubelet und Kubeproxy (vgl. Beispiel~\ref{fig:k3sarchitektur}).
Einige Besonderheiten von k3s zu k8s:

\subsubsection{\textit{Kine}}
ist eine Abstraktionsschicht von etcd, welche sqlite, Postgress, Mysql und dqlite übersetzt \cite{k3sgit}.

\subsubsection{\textit{Flannel}}
verwendet Flannel als überlagerndes Netzwerkmodell \cite{flannel}.

\begin{figure}
  \centering
  \includegraphics[width=1.0\columnwidth]{images/k3s.png}
  \caption{k3s Architektur \cite{k3s}}
  \label{fig:k3sarchitektur}
\end{figure}

\subsection{Rancher}
Die zuständige Firma hinter k3s ist das Softwareunternehmen Rancher.
